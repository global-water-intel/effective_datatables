  ...
  def datatable_header_filter(form, name, value, opts)
    ...
    when :string, :text, :number
      form.input name, label: false, required: false, value: value,
        ...
        input_html: { id: "uid-#{SecureRandom.hex}", name: nil, value: value, autocomplete: 'off', data: {'column-name' => opts[:name], 'column-index' => opts[:index]} }
    when :date
      form.input name, label: false, required: false, value: value,
        as: (ActionView::Helpers::FormBuilder.instance_methods.include?(:effective_date_picker) ? :effective_date_picker : :string),
        placeholder: placeholder,
        input_group: false,
        input_html: { id: "uid-#{SecureRandom.hex}", name: nil, autocomplete: 'off', data: {'column-name' => opts[:name], 'column-index' => opts[:index]} },
    ...
    when :bulk_actions_column
      form.input name, label: false, required: false, value: nil,
        as: :boolean,
        input_html: { id: "uid-#{SecureRandom.hex}", name: nil, value: nil, autocomplete: 'off', data: {'column-name' => opts[:name], 'column-index' => opts[:index], 'role' => 'bulk-actions-all'} }
    end
  end
